...
function readPosts() {
    fetch(`${API}?q=${searchValue}&_page=${currentPage}&_limit=6`) // получение данных из db.json
        ...
        .then((data) => {
          ...
          <img id="${item.id}" src="${item.postImg}" class="card-img-top detailsCard" style="height: 280px" alt="${item.postTitle}" />
        <div class="card-body">
          <h6 class="card-title">${item.postTitle}</h6>
          <p style="font-size: 12px;">Views: ${item.postVies}</p>
          <div style="display: flex;justify-content: space-around;
    align-items: center;" >
          <img width="25" height="25" src="../img/like.svg" onclick="addLike(${item.id})" alt="like">${item.postLike}
          <img width="20" height="20" src="../img/instagram-comment.svg" onclick="addCom(${item.id})" alt="comment">${item.postCom}
          <img width="25" height="25" src="../img/paper-plane.svg" onclick="addPlane(${item.id})" alt="plane">${item.postPlane}     
          </div>
